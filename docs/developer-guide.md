...
const receivedMessages =
  await ContractService.getReceivedMessages("5GrwvaEF...");
```
...
```typescript
const decryptedMedia = await UnlockService.unlockMessage(
  message,
  walletAccount
);
```
    ...
    "messages": [
      {
        "args": [
          {
            "label": "encrypted_key_cid",
            "type": { "displayName": ["String"], "type": 1 }
          },
          {
            "label": "encrypted_message_cid",
            "type": { "displayName": ["String"], "type": 1 }
          },
          {
            "label": "message_hash",
          ...
          {
            "label": "unlock_timestamp",
            "type": { "displayName": ["u64"], "type": 2 }
          },
          {
            "label": "recipient",
            "type": { "displayName": ["AccountId"], "type": 3 }
          }
        ],
...
const x25519PublicKey =
  sodium.crypto_sign_ed25519_pk_to_curve25519(ed25519PublicKey);
```
...
async function decryptKeyWithWallet(
  ...
  const { ephemeralPublicKey, iv, encryptedKey } =
